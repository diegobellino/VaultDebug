...
namespace VaultDebug.Editor.Console.Samples
{
    ...
    {
        private static ILoggerProvider _loggerProvider = new LoggerProvider();
        private static IVaultLogPool _logPool = new VaultLogPool();

        [MenuItem("Vault Debug/Console/Generate test logs")]
        public static void TestLogs()
        {
            var logger1 = _loggerProvider.GetLogger("Sample 1", _logPool);
            var logger2 = _loggerProvider.GetLogger("Sample 2", _logPool);
